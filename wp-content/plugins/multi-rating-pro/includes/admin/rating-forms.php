										...
										if ( count( $all_rating_items ) > 0 ) {
											?><ul><?php
												foreach ( $all_rating_items as $rating_item ) {
													?><li><input type="button" id="mrp-rating-item-<?php echo $rating_item['rating_item_id']; ?>" class="mrp-add-rating-item button button-secondary" value="<?php echo esc_html( $rating_item['description'] ); ?>"<?php if ( isset( $rating_form['rating_items'][$rating_item['rating_item_id']] ) ) { echo ' disabled=\"true\"'; } ?>/></li><?php
												}
											?></ul><?php
										} else {
