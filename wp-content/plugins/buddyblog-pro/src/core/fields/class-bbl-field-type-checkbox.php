 ...
 * Multi checkbox field type.
 */
class BBL_Field_Type_Checkbox extends BBL_Field_Type {
	...
	public function __construct( $args = array() ) {
		...
		$args = wp_parse_args(
			...
			array(
				'type'        => 'checkbox',
				'label'       => _x( 'Checkboxes', 'Field type label', 'buddyblog-pro' ),
				'description' => _x( 'Multiple checkbox field.', 'Field type label', 'buddyblog-pro' ),
				'supports'    => array(
					'multiple' => true,
					'settings' => true,
				),
	...
	public function sanitize_value( $meta_value, $field_settings, $form_id, $post_id = 0 ) {
		...
		return array_intersect( $vals, (array) $meta_value );
	}
	...
	public function edit_field_markup( $args ) {
		...
		<label class='bbl-field-label bbl-field-label-type-<?php echo esc_attr( $atts["type"] ); ?> bbl-field-label-field-<?php echo esc_attr( $atts["id"] ); ?>'>
			<?php echo $args['label']; ?>
			<?php echo $args['required']; ?>
		</label>
		<?php
		foreach ( $args['options'] as $option ) {
			echo  "<label><input " . $this->get_html_attributes($atts ) . ' ' . checked( in_array( $option['value'], (array) $args['value'] ), true, false ) . "  value='" . esc_attr( $option['value'] ) . "' /> {$option['label']}</label>";
		}
	}
	...
	public function admin_field_settings_markup( $args = array() ) {
		?>
		<div class="bbl-row bbl-row-cf bbl-row-cf-field-checkbox-options">
			<label class="bbl-label bbl-label-cf bbl-col-left">
				<?php _e( 'Options:', 'buddyblog-pro' ); ?>
			</label>
			<div class="bbl-col-right bbl-col-cf-field-checkbox-options">
				<input type="text" placeholder="<?php _e( 'e.g one,two,three', 'buddyblog-pro' ); ?>" name="bbl-input-cf-field-checkbox-options"/>
				<?php _e( 'Comma separated list of input options', 'buddyblog-pro' ); ?>
			</div>
		</div><!-- end of row -->

        <div class="bbl-row bbl-row-cf bbl-row-cf-field-checkbox-labels">
			<label class="bbl-label bbl-label-cf bbl-col-left">
				<?php _e( 'Option Labels:', 'buddyblog-pro' ); ?>
			</label>
			<div class="bbl-col-right bbl-col-cf-field-select-labels">
				<input type="text" placeholder="<?php _e( 'Labels', 'buddyblog-pro' ); ?>" name="bbl-input-cf-field-checkbox-labels"/>
