<?php
/**
 * All Posts list template page
 *
 * You can copy this to your w-content/themes/[your-active-theme]/buddyblog/community-builder/default/ and override if needed.
 *
...
<?php else : ?>
	<?php if ( bp_is_my_profile() && bblpro_user_can_create_post( get_current_user_id(), bblpro_get_current_post_type() ) ) : ?>
		<div class="bbl-notice bbl-create-post-notice">
			<p> <?php _e( "You haven't posted anything yet.", 'buddyblog-pro' ); ?> <a href="<?php echo esc_url( bblpro_get_post_type_create_tab_url( get_current_user_id(), bblpro_get_current_post_type() ) ); ?>"> <?php _e( 'Add New', 'buddyblog-pro' ); ?></a></p>
		</div>
	<?php else : ?>
