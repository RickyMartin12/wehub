...
class Post_Approval_Notifier {
	...
	private function setup() {
		add_action( 'bblpro_post_approved', array( $this, 'notify' ) );
	}
	...
	public function notify( $post_id ) {
		...
		$notify_author = get_post_meta( $form_id, '_buddyblog_notify_author_on_approval', true );

		if ( ! $notify_author ) {
			return; // no need to notify author.
		}

		$subject = get_post_meta( $form_id, '_buddyblog_approved_author_notification_subject', true );
		$message = get_post_meta( $form_id, '_buddyblog_approved_author_notification_message', true );
